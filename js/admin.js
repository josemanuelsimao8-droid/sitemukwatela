(() => {
  ...
  function bindContentEvents(){
    ...
    list.querySelectorAll('[data-save-content]').forEach(button=>button.addEventListener('click',async()=>{
      const key=decodeURIComponent(button.dataset.saveContent||'').split('|');
      const [oldPage,oldSection,oldField]=key;
      const fields=list.querySelectorAll('[data-content="'+button.dataset.saveContent+'"]');
      const values={page:oldPage,section:oldSection,field:oldField,value:''};
      fields.forEach(field=>{values[field.dataset.field]=field.value.trim();});
      const result=await db().from('site_content').update({page:values.page,section:values.section,field:values.field,value:values.value,updated_at:new Date().toISOString()}).eq('page',oldPage).eq('section',oldSection).eq('field',oldField);
      if(result.error){msg('Não foi possível guardar o conteúdo.','error');return;}
      msg('Conteúdo atualizado.','success');
      await loadContent();
    }));
  ...
  function bindPaymentEvents(){
    ...
    list.querySelectorAll('[data-save-payment]').forEach(button=>button.addEventListener('click',async()=>{
      ...
      if(!/^[a-z0-9_-]+$/.test(patch.code)){msg('O código deve conter apenas letras minúsculas, números, _ ou -.','error');return;}
      ...
      msg('Método atualizado. O código interno permanece estável para não afetar pedidos existentes.','success');
      await loadPayments();
    }));
  }
