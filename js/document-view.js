(() => {
  ...
  async function init(){
    ...
    const [settings,order]=await Promise.all([
      ...
      db().from('orders').select('order_number,notes,order_items(service_name,quantity,unit_price,subtotal,specifications)').eq('id',d.data.order_id).maybeSingle()
    ]);
