(() => {
  ...
  async function init(){
    ...
    const p=await db().from('profiles').select('id,full_name,phone,company,address,role,created_at').eq('id',id).maybeSingle();
    if(p.error||!p.data){msg('Cliente não encontrado.','error');return;}
    const o=await db().from('orders').select('id,order_number,status,total,currency,payment_status,created_at,order_items(service_name,quantity,item_type)').eq('user_id',id).order('created_at',{ascending:false}).limit(100);
    if(o.error){msg('Não foi possível carregar os pedidos do cliente.','error');return;}
    const x=p.data,orders=o.data||[];
    const roleLabel=x.role==='admin'?'Administrador':'Cliente';
    const root=document.getElementById('admin-client-detail');
    root.innerHTML='<div class="order-detail-head"><div><span class="eyebrow">Cliente</span><h1>'+esc(x.full_name||'Sem nome')+'</h1><p>Registado em '+esc(new Date(x.created_at).toLocaleString('pt-PT'))+'</p></div><div class="card-actions"><span class="admin-chip">'+esc(roleLabel)+'</span><a class="btn btn-secondary" href="admin.html">Voltar</a></div></div>'+
    ...
    document.getElementById('client-edit-form').addEventListener('submit',async e=>{
      e.preventDefault();
      const patch={full_name:document.getElementById('client-name').value.trim(),phone:document.getElementById('client-phone').value.trim()||null,company:document.getElementById('client-company').value.trim()||null,address:document.getElementById('client-address').value.trim()||null,updated_at:new Date().toISOString()};
      const r=await db().from('profiles').update(patch).eq('id',id);
      if(r.error){msg('Não foi possível atualizar o cliente.','error');return;}
      msg('Dados do cliente atualizados.','success');
    });
  }
