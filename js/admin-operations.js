(() => {
  ...
  async function loadQuotes(){
    ...
    if(ids.length){const x=await db().from('orders').select('id,order_number').in('id',ids);orders=x.data||[];const y=await db().from('order_items').select('order_id,service_name').in('order_id',ids);items=y.data||[];}
    ...
    items.forEach(x=>{im[x.order_id]=x.service_name});
    state.quotes=(r.data||[]).map(x=>({...x,order_number:om[x.order_id]?.order_number||'—',customer_name:pm[x.customer_id]?.full_name||'Cliente',service_name:im[x.order_id]||'Serviço'}));
    renderQuotes();
  }
  ...
  function renderDelivery(){
    ...
    root.innerHTML=rows.map(x=>'<article class="ops-admin-card"><div class="ops-admin-head"><div><span class="editor-kicker">'+esc(x.order_number)+'</span><h3>'+esc(x.profiles?.full_name||'Cliente')+'</h3><small>'+esc(x.order_items?.[0]?.service_name||'Serviço')+'</small></div><span class="admin-chip">'+esc(x.delivery_method==='delivery'?'Entrega':'Levantamento')+'</span></div><div class="field-two cms-field-grid"><div><label>Estado da entrega</label><select data-delivery="'+x.id+'"><option value="pending" '+(x.delivery_status==='pending'?'selected':'')+'>A preparar</option><option value="preparing" '+(x.delivery_status==='preparing'?'selected':'')+'>Em preparação</option><option value="ready" '+(x.delivery_status==='ready'?'selected':'')+'>Pronto</option><option value="out_for_delivery" '+(x.delivery_status==='out_for_delivery'?'selected':'')+'>Em entrega</option><option value="delivered" '+(x.delivery_status==='delivered'?'selected':'')+'>Entregue</option><option value="picked_up" '+(x.delivery_status==='picked_up'?'selected':'')+'>Levantado</option><option value="cancelled" '+(x.delivery_status==='cancelled'?'selected':'')+'>Cancelado</option></select></div><div><label>Contacto</label><input readonly value="'+esc(x.delivery_contact||'')+'"></div></div><div class="delivery-meta"><span><strong>Morada:</strong> '+esc(x.delivery_address||'Levantamento na Mukwatela')+'</span><span><strong>Valor:</strong> '+esc(money(x.total,x.currency))+'</span></div></article>').join('')||'<div class="operations-empty">Sem entregas/levantamentos encontrados.</div>';
